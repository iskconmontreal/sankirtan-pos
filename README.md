...
There is no sign-up. The sankirtan leader creates each devotee's user in Mandir (Users) with their email and assigns the **Book Distributor** role (`sankirtan:view` to browse + `sankirtan:create` to submit sessions). Google sign-in matches that email; email+OTP works as a fallback. Accounts without the role are refused by the POS.
